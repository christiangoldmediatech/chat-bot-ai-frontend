...
async function load(): Promise<void> {
  ...
  error.value = null
  try {
    lead.value = await superLeadsApi.get(leadId.value)
    form.value = {
    ...
    // Fetch the owning company in parallel so the header can link out to
    // it. Don't block the page on this — silent fallback if it fails.
    void loadCompany(lead.value.tenantId)
  } catch (err) {
...
async function loadCompany(tenantId: string): Promise<void> {
  try {
    company.value = await companiesApi.get(tenantId)
  } catch {
    company.value = null
  }
}
...
async function saveChanges(): Promise<void> {
  ...
  try {
    ...
    await superLeadsApi.update(leadId.value, payload)
...
async function retrySync(): Promise<void> {
  ...
  error.value = null
  try {
    await superLeadsApi.syncCrm(leadId.value)
...
<template>
  <div>
    <NuxtLink to="/superadmin/leads" class="inline-flex items-center gap-1.5 text-sm text-slate-400 hover:text-slate-200">
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="size-4">
        <polyline points="15 18 9 12 15 6" />
      </svg>
      {{ $t('superadmin.leads.backToAllLeads') }}
    </NuxtLink>
    ...
    <template v-else-if="lead">
      ...
      <header class="mt-4 rounded-2xl bg-slate-800/60 ring-1 ring-slate-700 p-5 flex items-start gap-4 flex-wrap">
        ...
        <div class="flex-1 min-w-[16rem]">
          ...
          <!-- Company chip — clickable so the superadmin can jump to the
               company detail or the per-company leads list. -->
          <div class="mt-2 flex items-center gap-2 flex-wrap text-xs">
            <span class="inline-flex items-center gap-1.5 rounded-full bg-slate-700/60 ring-1 ring-slate-600 px-2 py-0.5 text-slate-200">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="size-3 text-slate-400">
                <path d="M3 21h18M5 21V7l8-4v18M19 21V11l-6-4" />
              </svg>
              {{ company?.name ?? lead.tenantId.slice(0, 8) }}
            </span>
            <NuxtLink
              :to="`/superadmin/companies/${lead.tenantId}/leads`"
              class="text-indigo-300 hover:underline"
            >
              {{ $t('superadmin.leads.viewCompanyLeads') }} →
            </NuxtLink>
          </div>
      ...
      <div class="mt-4 grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div class="lg:col-span-2 space-y-4">
          ...
          <!-- Timeline (dark-themed inline render — same data as the
               admin LeadEventsTimeline component, just restyled for the
               superadmin layout). -->
          <section class="rounded-2xl bg-slate-800/60 ring-1 ring-slate-700 p-5">
