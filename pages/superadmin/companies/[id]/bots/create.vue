...
<template>
  <div>
    <NuxtLink :to="`/superadmin/companies/${tenantId}`" class="text-sm text-slate-400 hover:text-slate-200">{{ $t('superadmin.companyBotCreate.back') }}</NuxtLink>
    <h1 class="mt-2 text-2xl font-semibold text-slate-100 tracking-tight">{{ $t('admin.botCreate.title') }}</h1>
    ...
    <div v-if="!createdBot" class="mt-6 grid grid-cols-1 lg:grid-cols-[280px_minmax(0,40rem)] lg:items-start gap-6">
      ...
      <aside class="lg:sticky lg:top-6 rounded-2xl bg-slate-900/60 ring-1 ring-slate-800 p-5">
        <div class="flex items-center gap-2">
          ...
          <h2 class="text-sm font-semibold text-slate-100">{{ $t('admin.botCreate.metaGuide.title') }}</h2>
        </div>
        ...
        <ol class="mt-4 space-y-3">
          <li class="flex gap-2.5">
            ...
            <div class="min-w-0">
              <p class="text-xs font-semibold text-slate-100">{{ $t('admin.botCreate.metaGuide.step1Title') }}</p>
              <p class="text-[11px] text-slate-500 mt-0.5">
                <a href="https://developers.facebook.com/apps" target="_blank" rel="noreferrer" class="text-indigo-300 hover:underline">developers.facebook.com</a> {{ $t('admin.botCreate.metaGuide.step1Body') }}
              </p>
            </div>
          </li>
          <li class="flex gap-2.5">
            ...
            <div class="min-w-0">
              <p class="text-xs font-semibold text-slate-100">{{ $t('admin.botCreate.metaGuide.step2Title') }}</p>
          ...
          <li class="flex gap-2.5">
            ...
            <div class="min-w-0">
              <p class="text-xs font-semibold text-slate-100">{{ $t('admin.botCreate.metaGuide.step3Title') }}</p>
              <p class="text-[11px] text-slate-500 mt-0.5">{{ $t('admin.botCreate.metaGuide.step3BodyBefore') }}<span class="font-medium text-slate-300">{{ $t('admin.botCreate.metaGuide.step3BodyEmph') }}</span>{{ $t('admin.botCreate.metaGuide.step3BodyAfter') }}</p>
            </div>
          </li>
          <li class="flex gap-2.5">
            ...
            <div class="min-w-0">
              <p class="text-xs font-semibold text-slate-100">{{ $t('admin.botCreate.metaGuide.step4Title') }}</p>
              <p class="text-[11px] text-slate-500 mt-0.5">
                {{ $t('admin.botCreate.metaGuide.step4Body') }}
              </p>
            </div>
          </li>
          <li class="flex gap-2.5">
            ...
            <div class="min-w-0">
              <p class="text-xs font-semibold text-slate-100">{{ $t('admin.botCreate.metaGuide.step5Title') }}</p>
          ...
          <li class="flex gap-2.5">
            ...
            <div class="min-w-0">
              <p class="text-xs font-semibold text-slate-100">{{ $t('admin.botCreate.metaGuide.step6Title') }}</p>
              <p class="text-[11px] text-slate-500 mt-0.5">{{ $t('admin.botCreate.metaGuide.step6BodyBefore') }}{{ $t('admin.botCreate.metaGuide.step6Link') }}{{ $t('admin.botCreate.metaGuide.step6BodyAfter') }}</p>
            </div>
          </li>
        </ol>
      ...
      <section class="rounded-2xl bg-slate-900/60 ring-1 ring-slate-800 p-6 space-y-5">
        <header class="flex items-start gap-3">
          ...
          <div>
            <h2 class="text-base font-semibold text-slate-100">{{ $t('admin.botCreate.section.detailsTitle') }}</h2>
            <p class="text-xs text-slate-500 mt-0.5">{{ $t('admin.botCreate.section.detailsSubtitle') }}</p>
          </div>
        </header>

        <div>
          <label for="name" class="block text-sm font-medium text-slate-300">{{ $t('admin.botCreate.section.nameLabel') }}</label>
          <input
            id="name"
            v-model="form.name"
            type="text"
        ...
        <div>
          <label for="systemPrompt" class="block text-sm font-medium text-slate-300">{{ $t('admin.botCreate.section.systemPromptLabel') }}</label>
          <textarea
            id="systemPrompt"
        ...
        <div class="flex items-center gap-2">
          ...
          <label for="isActive" class="text-sm text-slate-300">{{ $t('admin.botCreate.section.activateLabel') }}</label>
        </div>
      </section>
      ...
      <section class="rounded-2xl bg-slate-900/60 ring-1 ring-slate-800 p-6 space-y-5">
        <header class="flex items-start gap-3">
          ...
          <div>
            <h2 class="text-base font-semibold text-slate-100">{{ $t('admin.botCreate.section.waConnectionTitle') }}</h2>
            <p class="text-xs text-slate-500 mt-0.5">
              {{ $t('admin.botCreate.section.waConnectionSubtitleBefore') }}
              <a href="https://developers.facebook.com/apps" target="_blank" rel="noreferrer" class="font-medium text-indigo-300 hover:underline">{{ $t('admin.botCreate.section.waConnectionSubtitleLink') }}</a>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <label for="phoneId" class="block text-sm font-medium text-slate-300">{{ $t('admin.botCreate.section.phoneIdLabel') }}</label>
            <input
              id="phoneId"
          ...
          <div>
            <label for="wabaId" class="block text-sm font-medium text-slate-300">
              {{ $t('admin.botCreate.section.wabaIdLabel') }} <span class="text-slate-500 font-normal">{{ $t('admin.botCreate.section.wabaIdOptional') }}</span>
            </label>
            <input
              id="wabaId"
        ...
        <div>
          <label for="token" class="block text-sm font-medium text-slate-300">{{ $t('admin.botCreate.section.tokenLabel') }}</label>
          <input
            id="token"
        ...
        <div>
          <label for="appSecret" class="block text-sm font-medium text-slate-300">
            App secret <span class="text-slate-500 font-normal">{{ $t('admin.botCreate.section.wabaIdOptional') }}</span>
          </label>
          <input
            id="appSecret"
        ...
        <div>
          <label for="verify" class="block text-sm font-medium text-slate-300">{{ $t('admin.botCreate.section.verifyLabel') }}</label>
    ...
    <template v-else>
      <div class="mt-6 max-w-2xl rounded-2xl border border-success-800 bg-success-950 p-4 text-sm text-success-300">
        {{ $t('admin.botCreate.postCreate.successBefore') }}<strong class="font-semibold text-white">{{ createdBot.name }}</strong>{{ $t('admin.botCreate.postCreate.successAfter') }}
      </div>
